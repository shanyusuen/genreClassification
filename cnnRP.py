...
x_train, y_train = customRead.read("out/strat.train.arff")
x_train = np.array([np.reshape(x_train[i], (24, 60, 1))
                   for i in range(len(x_train))])

x_test, y_test = customRead.read("out/strat.test.arff")
x_test = np.array([np.reshape(x_test[i], (24, 60, 1))
                   for i in range(len(x_test))])

y_train = keras.utils.to_categorical([transform(y) for y in y_train],
                                     len(categories))
y_test = keras.utils.to_categorical([transform(y) for y in y_test],
                                    len(categories))
